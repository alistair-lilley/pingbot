...
class Pinger:
    def __init__(self, guild, channel, username):
        if (
            "custom_dialogs.json" not in os.listdir()
            or os.stat("custom_dialogs.json").st_size == 0
        ):
        ...
        if self.guild in self.guild_vals:
            self.timeout_max = self.guild_vals[str(self.guild.id)]["timeout"] * MINUTE
        else:
    ...
    async def start_pinging(self):
        if self.guild and not self._verify_user():
            await self.channel.send(
                re.sub(dialogs["user"], self.target_user, dialogs["not_in_server"])
            )
            return
        start = time.time()
    ...
    async def _ping_repeating(self, end):
        while time.time() < end and not self.stop_ping:
            ...
            pingmsg = re.sub(dialogs["user"], self.target_user, dialogs["ping_message"])
